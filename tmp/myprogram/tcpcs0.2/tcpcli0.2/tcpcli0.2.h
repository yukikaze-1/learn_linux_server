...
#define TCP_CLI_H_
    ...
    void Send_To_Serv(int sockfd,int file_fd,const std::string& filename);
    //send the file linfo to the server and wait for the respon
    int send_message(int sock_fd,const struct stat &info,const std::string& filename);
    //send the file
    ...
    //remember to rewrite the exit and return
