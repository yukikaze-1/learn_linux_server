...
#define TCP_CLI_H_
    ...
    void Get_Host_Info(struct sockaddr_in& cliaddr);

    //read the message and file in scoket
    //retrun the new file fd
    int read_store_file(int connfd,std::string& filename);
    ...
    void write_back_file(int connfd,int file_fd,const std::string& filename);
